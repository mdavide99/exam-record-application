    ...
    public ExamSession findAllSubscribedStudents(int SessionID, String tableOrder, boolean ascending, int teacherId) throws SQLException{
        ...
        switch(tableOrder){
            ...
            case "DegreeCourse":
                ...
                break;
            case "Result":
                if(ascending) query = query + " ORDER BY IF(R.Result RLIKE '^[A-Z]', 1, 2), R.Result";
                else query = query + " ORDER BY IF(R.Result RLIKE '^[A-Z]', 1, 2) DESC, R.Result DESC";
                break;
            case "State":
                query = query + " ORDER BY R.State";
                if(!ascending) query = query + " DESC";
                break;
            default:
                break;
